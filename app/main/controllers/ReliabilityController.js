'use strict';
angular.module('main')
  .controller('ReliabilityController', function ($scope, $state, $cordovaDeviceMotion, $timeout, $log) {
    var vm = this;
    ...
    function navigate () {
      $log.log('ReliabilityController navigate called : vm.currAcceleration ', vm.currAcceleration);
      ...
      if (vm.currAcceleration.y > 9) {
        ...
        $timeout.cancel(vm.timer);
        vm.watch.clearWatch();
        $state.go('menu', {stateId: 'reliability'});
      }
    }
    ...
    $scope.$on('$ionicView.beforeLeave', function () {
      $log.log('ReliabilityController $ionicView.beforeLeave : watch ', vm.watch);
      if (vm.watch) {
        vm.cancelWatch = true;
        $timeout.cancel(vm.timer);
        vm.watch.clearWatch();
    ...
    $scope.$on('$destroy', function () {
      $log.log('ReliabilityController $destroy : watch ', vm.watch);
      if (vm.watch) {
        vm.cancelWatch = true;
        $timeout.cancel(vm.timer);
        vm.watch.clearWatch();
