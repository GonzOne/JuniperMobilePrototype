'use strict';
angular.module('main')
  .controller('ManageController', function ($scope, $state, $cordovaDeviceMotion, $timeout, $log) {
    var vm = this;
    ...
    function navigate () {
      $log.log('ManageController navigate called : vm.currAcceleration ', vm.currAcceleration);
    ...
    $scope.$on('$ionicView.beforeLeave', function () {
      $log.log('ManageController $ionicView.beforeLeave : watch ', vm.watch);
      if (vm.watch) {
        vm.cancelWatch = true;
        $timeout.cancel(vm.timer);
        vm.watch.clearWatch();
    ...
    $scope.$on('$destroy', function () {
      $log.log('ManageController $destroy : watch ', vm.watch);
      if (vm.watch) {
        vm.cancelWatch = true;
        $timeout.cancel(vm.timer);
        vm.watch.clearWatch();
