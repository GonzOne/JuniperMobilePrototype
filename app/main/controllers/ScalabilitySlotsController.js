'use strict';
angular.module('main')
  .controller('ScalabilitySlotsController', function ($scope, $state, $cordovaDeviceMotion, $timeout, $log) {
    var vm = this;
    ...
    function navigate () {
      $log.log('ScalabilitySlotsController navigate called : vm.currAcceleration ', vm.currAcceleration);
      ...
      if (vm.currAcceleration.y > 9) {
        ...
        $timeout.cancel(vm.timer);
        vm.watch.clearWatch();
        $state.go('menu', {stateId: 'scalabilitySlots'});
      }
    }
    ...
    $scope.$on('$ionicView.beforeLeave', function () {
      $log.log('ScalabilitySlotsController $ionicView.beforeLeave : watch ', vm.watch);
      if (vm.watch) {
        vm.cancelWatch = true;
        $timeout.cancel(vm.timer);
        vm.watch.clearWatch();
    ...
    $scope.$on('$destroy', function () {
      $log.log('ScalabilitySlotsController $destroy : watch ', vm.watch);
      if (vm.watch) {
        vm.cancelWatch = true;
        $timeout.cancel(vm.timer);
        vm.watch.clearWatch();
